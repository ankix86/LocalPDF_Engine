...
export default function PDFToJPGPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      // Show first 4 as previews
      const thumbs: string[] = [];
      for (const img of images.slice(0, 4)) {
        thumbs.push(URL.createObjectURL(img.blob));
      }
      setPreviews(thumbs);
  ...
  return (
    <ToolLayout
      title="PDF to JPG"
      description="Export each page as a high-resolution image."
      icon="image"
    ...
    >
      <div className="space-y-4">
        <ToolCard>
          <FileDropzone
            onFiles={(f) => { setFile(f[0]); setDone(false); setPreviews([]); }}
            files={file ? [file] : []}
          />
        </ToolCard>
        ...
        {file && !done && (
          <ToolCard>
            <div className="grid sm:grid-cols-2 gap-6">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-700 mb-2">
                  Resolution: {scale === 1 ? "72 dpi" : scale === 1.5 ? "108 dpi" : scale === 2 ? "144 dpi" : "216 dpi"}
                </p>
                <div className="flex gap-2">
                  {[1, 1.5, 2, 3].map((s) => (
                    <button
            ...
            {processing && (
              <div className="mt-4">
                <ProgressBar value={progress} label={`Converting pages… ${progress}%`} />
              </div>
            )}
        ...
        {done && (
          <ToolCard>
            <div className="flex flex-col items-center gap-4 py-6">
              ...
              <p className="font-semibold text-slate-800">Downloaded as ZIP!</p>
              ...
              <button
                onClick={reset}
                className="mt-2 inline-flex items-center gap-2 bg-white border border-slate-300 text-slate-700 text-sm font-medium px-5 py-2.5 rounded hover:bg-slate-50 transition-colors"
              >
